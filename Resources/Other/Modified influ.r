...
#' Initialize the Influence object
#'
#' @param obj The influence object to initialize, e.g., the output of influence(model, data=mydata, ...)
#' @return The initialized influence object
  ...
  if (inherits(obj$model, "gam")) {
    ...
    if (length(smooth_terms) > 0) {
      ...
      for (i in 1:length(obj$model$smooth)) {
        ...
        # Check for interactions with year
        if (inherits(obj$model$smooth[[i]], "tensor.smooth") || 
          ...
          # Check if this is an interaction involving the focus term
          if (length(vars) > 1) {
            # Store interaction information
    ...
    # Check for interactions with year in GLM terms
    obj$interactions <- list()
...
#' Extract coefficients for a term from a GAM
#'
#' @param obj The influence object
...
#' Extract term effects for a specific term, handling interactions
#'
#' @param obj The influence object
  ...
  if (is_interaction || (term == obj$focus && has_interactions_with_focus)) {
    ...
    if (term %in% colnames(preds$fit)) {
      ...
      if (obj$interactions[[term]]$type %in% c("tensor.smooth", "tp", "ti")) {
        ...
        if (obj$focus %in% interaction_vars) {
          ...
          # This is an interaction not involving the focus term
          effects <- preds$fit[, term]
        }
      }
    } else {
    ...
    if (term == obj$focus) {
      # For the focus term, return the effects directly
      return(c(0, unique(effects)))
    } else {
    ...
    # For non-interaction terms or terms not involving the focus
...
#' Create a prediction grid for a term, handling interactions
#'
#' @param obj The influence object
#' @param term The term to create a grid for
#' @return A data frame with the prediction grid
#' @keywords internal
create_prediction_grid <- function(obj, term) {
  # Initialize an empty list to store the grid variables
  grid_vars <- list()
  
  # Always include the focus variable
  ...
  # For all other terms involved in interactions with the focus
  if (term == obj$focus) {
    for (interaction_name in names(obj$interactions)) {
      interaction_info <- obj$interactions[[interaction_name]]
      if (obj$focus %in% interaction_info$vars) {
...
#' @export
get_ses <- function(obj, model = obj$model, term = obj$focus) {
  UseMethod("get_ses", model)
}
...
#' Calculate necessary values for plots and statistics
#'
#' This method must be called before any plots can be generated
#'
#' @param obj The influence object
#' @param islog Whether the response is log transformed
#' @return The updated influence object
#' @export
calc <- function(obj, islog = NULL) {
  ...
  if (inherits(obj$model, "gam")) {
    ...
    for (i in seq_along(obj$model$smooth)) {
      ...
      # Check if this is an interaction term involving the focus
      if (term %in% names(obj$interactions) && 
    ...
    if (has_interactions) {
      ...
      # Process interactions involving the focus
      for (term in names(obj$interactions)) {
        ...
        if (obj$focus %in% interaction_info$vars || "year" %in% interaction_info$vars) {
          ...
          if (term %in% colnames(pred_grid$fit)) {
            ...
            # Map values to original data by matching focus levels
            mapped_values <- numeric(nrow(obj$data))
            mapped_se <- numeric(nrow(obj$data))
            
            for (level in levels(obj$data[[obj$focus]])) {
              level_rows <- obj$data[[obj$focus]] == level
              grid_rows <- pred_data[[obj$focus]] == level
...
step_plot <- function(obj, panels = TRUE, ...) {
  ...
  if (panels) {
    ...
    for (i in seq_along(cols)) {
      ...
      plot_data <- obj$indices %>%
        select(level, all_of(names(obj$indices)[cols[1]:col])) %>%
        mutate(level_num = as.integer(level)) %>%
        pivot_longer(
    ...
    plot_data <- obj$indices %>%
      select(level, all_of(names(obj$indices)[cols])) %>%
      mutate(level_num = as.integer(level)) %>%
      pivot_longer(
        cols = -c(level, level_num),
        names_to = "step",
...
influ_plot <- function(obj, panels = TRUE, ...) {
  ...
  if (panels) {
    ...
    plot_data <- obj$influences %>%
      select(level, all_of(names(obj$influences)[cols])) %>%
      mutate(level_num = as.integer(level)) %>%
      pivot_longer(
        cols = -c(level, level_num),
        names_to = "variable",
...
infl_glm <- influence(gam.model, focus = "year")
infl_glm <- calc(infl_glm)
step_plot(infl_glm)
