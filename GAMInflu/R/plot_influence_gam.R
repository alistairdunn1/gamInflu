...
plot.influence_gam <- function(x, y = NULL, # Add y for generic compatibility
  ...
  # CDI Plot (Experimental for GAMs)
    ...
    p_distr <- ggplot(cdi_data$distr, aes(x = term_int, y = focus_int)) +
               ...
               theme(panel.grid.major = element_line(color = "grey90"),
                     ...
                     plot.margin = margin(t = 0, r = 5, b = 5, l = 5)) # Adjust margins
  ...
  # Step + Influence Plot
    ...
    combined_plot <- combined_plot +
                     plot_annotation(title = "Step and Influence Plots",
  ...
  # Interaction Plot
    ...
    p <- ggplot(plot_data, aes(x = x_factor, y = response_vals,
         ...
         labs(title = paste("Interaction Plot:", term_name),
...
prepare_step_data <- function(indices_df, focus_var) {
  ...
  if(length(step_cols) == 0) return(NULL)
  ...
  step_data_long <- tryCatch(
      ...
      error = function(e) {warning("tidyr::pivot_longer failed for step data: ", e$message); NULL}
  )
  if(is.null(step_data_long)) return(NULL)
...
prepare_influ_data <- function(influences_df, focus_var) {
  ...
  if(length(influ_cols) == 0) return(NULL)
  ...
  influ_data_long <- tryCatch(
      ...
      error = function(e) {warning("tidyr::pivot_longer failed for influence data: ", e$message); NULL}
  )
   if(is.null(influ_data_long)) return(NULL)
...
prepare_cdi_data <- function(obj, term) {
    ...
    return(list(coeffs = coeffs_data,
                distr = distr_data,
...
prepare_interaction_data <- function(obj, term) {
    ...
    all_term_preds <- tryCatch(stats::predict(obj$model, newdata = pred_grid, type = "terms", se.fit = FALSE),
                               error = function(e) { stop("Failed to predict terms for interaction plot: ", e$message) })
    ...
    if (term %in% colnames(all_term_preds)) {
        ...
        if (term_reversed %in% colnames(all_term_preds)) {
            ...
            if(length(matching_smooth) == 1) term_to_use <- matching_smooth
