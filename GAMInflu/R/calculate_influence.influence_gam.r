...
#' @param islog Deprecated. Logic now tries to infer if response is logged.
...
calculate_influence.influence_gam <- function(obj, islog = NULL, verbose = TRUE, ...) {
  ...
  if(!is.null(islog)) logged <- islog # Allow override, though discouraged
  ...
  # Calculate relative effect (exponentiated, centered)
  ...
  model_step0 <- tryCatch(do.call(gam, c(list(formula = formula_step0), gam_args)),
                           error = function(e) {warning("Failed to fit intercept-only model: ", e$message, call.=FALSE); NULL})
  ...
  for (i in 1:length(terms_rhs)) {
      ...
      model_step <- tryCatch(do.call(gam, c(list(formula = formula_step), gam_args)),
                              error = function(e) {warning("Failed to fit model at step '", step_name, "': ", e$message, call.=FALSE); NULL})
